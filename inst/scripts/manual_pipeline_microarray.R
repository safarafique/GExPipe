# ==============================================================================
# GExPipe MANUAL PIPELINE - MICROARRAY, single dataset (plain R, NOT Shiny)
#
...
#
# DE method: microarray uses limma (the standard, appropriate engine for
# continuous log-intensity data) - there is no method choice here. For a
# choice of 3 DE engines, see manual_pipeline_rnaseq.R instead.
# ==============================================================================
...
# STEP 1: USER INPUT
# ==============================================================================
gse_id <- "GSE268456"   # microarray GEO series accession
...
# STEP 2: Download
# ==============================================================================
dl <- gexp_download_one_microarray_gse(gse_id, work_dir)
if (!isTRUE(dl$ok)) stop("Download failed: ", dl$reason)
expr_raw <- dl$micro_expr
pdata    <- dl$metadata
...
# ==============================================================================
# STEP 3: Normalize
# Quantile-normalizes this dataset (same function/default the app's Step 2
# uses for microarray). Add more GSEs to micro_expr_list below to normalize
# and intersect several datasets together instead of just one.
# ==============================================================================
micro_expr_list <- setNames(list(expr_raw), gse_id)
...
expr_norm <- norm_out$combined_expr   # normalized genes x samples matrix
...
# With a single dataset there is no between-study batch to remove - this
# just applies the variance-percentile gene filter (same default as the
# Shiny app's Step 5). Add more GSEs to micro_expr_list in Step 3 to enable
# real ComBat-ref/limma correction across studies.
# ==============================================================================
batch_out <- gexp_batch_correct(
  expr                = expr_norm,
  metadata            = meta,
  variance_percentile = 25,          # same default as the app's Step 5 slider
  method              = "combat_ref" # combat_ref | limma | sva | combat | quantile_limma | hybrid
)
...
write.csv(de_results, file.path(work_dir, paste0(gse_id, "_DE_results.csv")), row.names = FALSE)

# ==============================================================================
# STEP 8: WGCNA (weighted gene co-expression network)
# Uses the same prep helper as the app (outlier/NA filtering, top-variable
# gene selection), then the standard WGCNA package workflow: pick a soft
# power, build one merged network, extract module colors and eigengenes.
# ==============================================================================
library(WGCNA)
