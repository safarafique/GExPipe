# ==============================================================================
# GExPipe MANUAL PIPELINE - RNA-SEQ, 2+ DATASETS (plain R, NOT Shiny)
#
# Same as manual_pipeline_rnaseq.R but for combining any number of RNA-seq
# GSEs (2, 3, 4, 5, ...) into one analysis - download, normalize + intersect
# genes (raw counts kept aligned for count-based DE), QC, assign groups per
# dataset, real batch correction across studies, DE (choice of 3 engines),
# WGCNA, common genes + GO/KEGG, PPI.
#
...
# ==============================================================================
# STEP 1: USER INPUT - list as many RNA-seq GSEs as you need (2, 3, 4, ...)
...
pdata_list      <- list()
for (gse in gse_ids) {
  dl <- gexp_download_one_rnaseq_gse(gse, work_dir)
  if (!isTRUE(dl$ok)) {
    warning(gse, " failed: ", dl$reason)
    next
  }
  rna_counts_list[[gse]] <- dl$count_matrix
  pdata_list[[gse]]      <- dl$metadata
  cat(gse, ":", nrow(dl$count_matrix), "genes x", ncol(dl$count_matrix), "samples downloaded\n")
}
if (length(rna_counts_list) < 2L) {
...
## TMM -> log-CPM for the continuous matrix (QC/WGCNA); raw counts are kept
## separately, aligned across all datasets, for count-based DE.
## ============================================================================
...
expr_norm         <- norm_out$combined_expr        # normalized (log-CPM), all datasets, common genes
counts_for_deseq2 <- norm_out$raw_counts_for_deseq2 # raw integer counts, aligned across all datasets
...
## before correction) and hierarchical clustering. Uses the normalized
## log-CPM matrix, not raw counts.
## ============================================================================
...
## ============================================================================
## STEP 6: Batch correction (applied to the normalized log-CPM matrix only -
## used later for WGCNA/QC. Raw counts stay untouched for DE: DESeq2/edgeR/
## limma-voom keep raw counts and add Dataset as a covariate instead, same
## as the app.) Auto-picks the same way as the app's Step 5 recommendation.
## ============================================================================
...
## Always runs on the RAW counts (counts_for_deseq2), never the batch-
## corrected matrix. With 2+ datasets, gexpipe_run_count_de() automatically
## adds Dataset as a covariate in the design (same as the app).
## ============================================================================
...
write.csv(de_results, file.path(work_dir, paste0(de_method, "_DE_results.csv")), row.names = FALSE)
