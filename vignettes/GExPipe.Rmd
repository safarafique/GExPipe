...
This vignette is written for **end users** of the Shiny app (not package
developers). Developer-oriented vignettes (`gexpipe-workflow`,
`gexpipe-shiny-testing-ci`, `gexpipe-shiny-architecture`) are **not**
included in the package; they were removed to reduce installation and
check time. Maintainer notes live in the [GitHub
repository](https://github.com/safarafique/GExPipe).
...
For Shiny-app documentation conventions, see the [Bioconductor Shiny
apps
...
Screenshots of key app panels are embedded below using
`knitr::include_graphics()` and PNG files under
`inst/extdata/vignette-screenshots/`, following the [Bioconductor Shiny
documentation
pattern](https://contributions.bioconductor.org/shiny.html#documentation).
When PNGs are not bundled, the walkthrough text still describes each
step; add files named `step1_download.png`, `step2_qc.png`,
`step6_de.png`, `step9_ppi.png`, and `step15_summary.png` to show
figures in the HTML vignette.
...
Bundled CSV files in `inst/extdata/` are loaded with `system.file()` (no
path guessing or multiple fallbacks):
