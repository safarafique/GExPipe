...
install)`. If you only see `installed GExPipe package`, **restart R** once
(RStudio: `Ctrl+Shift+F10`), install `pkgload`, and run the command again —
otherwise an older Bioconductor install is used and probe-ID fixes are
skipped. The same restart after any install or update avoids Windows DLL
lock issues and ensures machine-learning helpers load correctly.
...
`ASHGV40000001`, or `AB000409` in the sample row list, wait for STEP 2b to
...
| **IDs still probe-like in log** | Restart R; use the [Run without installing](#installation) block above — an old Bioconductor install overrides GitHub fixes. |
