...
This vignette walks through the **GExPipe Shiny application** end to end.
For mixed microarray studies (Arraystar, Affymetrix, Agilent, custom probe
IDs), the download step automatically runs **STEP 2b** to map probes to HGNC
gene symbols before merging — see
...
**Run without installing** (always uses the latest `main` from GitHub):
...
The console should show `Mode: GitHub checkout (loaded from clone, not old
install)`. If you only see `installed GExPipe package`, restart R, install
`pkgload`, and run the command again — otherwise an older Bioconductor
install is used and probe-ID fixes are skipped.
...
- Enter GSE accessions (comma-separated). For a first test, use one series
  ...
  (probe → symbol conversion).
- Click **Download & merge**.

**Success check:** The log lists each GSE with detected ID format and
`Common genes` > 0. If you see probe IDs such as `(+)E1A_r60_1`,
`ASHG19AP…`, or `AB000409` in the sample row list, wait for STEP 2b to
finish or see [Troubleshooting](#troubleshooting).
...
fin <- gexp_download_finalize_common_genes(
  ...
  all_genes_list  = gexp_rebuild_all_genes_list(
    overlap_out$micro_expr_list,
    overlap_out$rna_counts_list
  )
)
...
| Problem | Solution |
|------------------------------------|------------------------------------|
| **0 common genes after download** | Check STEP 2b in the log. Probe/custom IDs must convert to symbols. Install `org.Hs.eg.db` and `biomaRt`. For Arraystar (`GSE188653`), allow GPL26963 to download once (cached under `micro_data/`). |
| **IDs still probe-like in log** | Restart R; use `pkgload` + `runGitHub(..., ref = "main")` or `pkgload::load_all()` on a fresh clone — an old Bioconductor install overrides GitHub fixes. |
