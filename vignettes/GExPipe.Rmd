...
batch effects, discover differentially expressed genes, build
co-expression networks, refine candidates with PPI and machine learning,
and export a clinical summary report — without manually wiring separate
Bioconductor tools together.
...
and `R CMD check` time manageable; see the [GitHub
repository](https://github.com/safarafique/GExPipe) for contributor
documentation.
...
matches a supported Bioconductor release (e.g. Bioconductor 3.22 for R
4.6).
...
`runGExPipe()` builds the app object and may install missing
dependencies on first run. `shiny::runApp()` starts the local server.
...
if (.vig_has_screenshot("step1_download.png")) {
...
if (.vig_has_screenshot("step2_qc.png")) {
...
if (.vig_has_screenshot("step6_de.png")) {
...
if (.vig_has_screenshot("step9_ppi.png")) {
...
if (.vig_has_screenshot("step15_summary.png")) {
