...
The resulting `GExPipe_0.99.13.tar.gz` is valid for submission; vignette source is in `vignettes/`. To build the vignette locally, install [Pandoc](https://pandoc.org/installing.html) and run `R CMD build .` without `--no-build-vignettes`.
...
```bash
R CMD build . --no-build-vignettes
R CMD check GExPipe_0.99.13.tar.gz --no-build-vignettes --no-manual
```

Before submission, fix any **ERROR**s and **WARNING**s from `R CMD check`, then run `BiocCheck("GExPipe_0.99.13.tar.gz")` and address any reported issues.
