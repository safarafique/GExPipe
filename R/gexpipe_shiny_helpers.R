...
probe_ids_to_symbol_gpl <- function(probe_ids, gpl_id) {
  ...
  symbol_col <- NULL
  for (cand in c("Gene Symbol", "GENE_SYMBOL", "Gene.symbol", "Symbol", "gene_symbol",
...
probe_ids_to_symbol_biomart <- function(probe_ids, gpl_id = NULL) {
  ...
  if (length(unique_ids) == 0) return(NULL)
...
map_microarray_ids <- function(micro_expr, fdata, micro_eset = NULL, gse_id = NULL) {
  ...
  if (!is.null(fdata) && ncol(fdata) > 0) {
    for (col in possible_symbol_cols) {
      if (col %in% colnames(fdata)) { gene_symbol_col <- col; break }
    }
  }
  ...
  if (!is.null(annot_pkg) && !is.na(annot_pkg) && nzchar(annot_pkg) && requireNamespace(annot_pkg, quietly = TRUE)) {
    ...
    gene_symbols <- tryCatch({
      if (is.null(db)) NULL else AnnotationDbi::mapIds(db, keys = probe_ids, column = "SYMBOL",
  ...
  if (is_likely_symbol && !any(grepl("^[0-9]{5,}", sample_ids))) {
    verified <- tryCatch({
      hs_db <- .gexpipe_hs_db()
      mapped <- AnnotationDbi::mapIds(hs_db, keys = head(sample_ids, 10), column = "SYMBOL",
  ...
  if (any(grepl("^ENSG", sample_ids))) {
    ...
    gene_symbols <- tryCatch({
      hs_db <- .gexpipe_hs_db()
      AnnotationDbi::mapIds(hs_db, keys = clean_keys, column = "SYMBOL",
  ...
  if (mean(grepl("^[0-9]+$", sample_ids), na.rm = TRUE) > 0.7) {
    gene_symbols <- tryCatch({
      hs_db <- .gexpipe_hs_db()
      AnnotationDbi::mapIds(hs_db, keys = as.character(probe_ids), column = "SYMBOL",
...
any_id_to_symbol <- function(ids, gpl_id = NULL) {
  ...
  if (any(grepl("^ENSG", sample_ids))) {
    ...
    sym <- tryCatch({
      hs_db <- .gexpipe_hs_db()
      suppressMessages(AnnotationDbi::mapIds(hs_db, keys = clean, column = "SYMBOL",
                                            keytype = "ENSEMBL", multiVals = "first"))
    }, error = function(e) NULL)
  ...
  if (mean(grepl("^[0-9]+$", sample_ids), na.rm = TRUE) > 0.3) {
    ...
    sym <- tryCatch({
      hs_db <- .gexpipe_hs_db()
      suppressMessages(AnnotationDbi::mapIds(hs_db, keys = keys_entrez, column = "SYMBOL",
                                            keytype = "ENTREZID", multiVals = "first"))
    }, error = function(e) NULL)
  ...
  sym <- tryCatch({
    hs_db <- .gexpipe_hs_db()
    suppressMessages(AnnotationDbi::mapIds(hs_db, keys = ids, column = "SYMBOL",
                                          keytype = "ALIAS", multiVals = "first"))
  }, error = function(e) NULL)
  if (!is.null(sym) && sum(!is.na(sym)) > length(ids) * 0.1) return(as.character(sym))
...
get_platform_for_gse <- function(gse_id) {
  ...
  if (length(platforms) == 0) return(NULL)
