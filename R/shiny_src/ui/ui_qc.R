...
ui_qc <- tabItem(
  ...
  fluidRow(
    box(
      title = tags$span(icon("venus-mars"), " Venn Diagram - Gene Overlap"),
    ...
    box(
      title = tags$span(icon("project-diagram"), " UpSet Plot - Gene Intersections"),
  ...
  fluidRow(
    box(
      title = tags$span(icon("chart-line"), " Quality Control Plots"),
  ...
  fluidRow(
    box(
      title = tags$span(
        icon("search"), " Sample Outlier Detection",
        tags$span("BEFORE NORMALIZATION",
