...
server_gsea <- function(input, output, session, rv) {
  ...
  output$gsea_placeholder_ui <- renderUI({
    ...
    if (!is.null(expr) && (is.matrix(expr) || is.data.frame(expr)) && nrow(expr) > 0) {
      return(NULL)
    }
    tags$div(
  ...
  output$gsea_collection_info_ui <- renderUI({
    ...
    labels <- c(
      H = "Hallmark", C5_BP = "GO:BP", C5_MF = "GO:MF", C2_KEGG = "KEGG",
      C2_REACTOME = "Reactome", C7 = "Immunologic", C6 = "Oncogenic"
    )
    approx_sizes <- c(
      H = "~50", C5_BP = "~7,700", C5_MF = "~1,700", C2_KEGG = "~186",
    ...
    tags$div(
      ...
      tags$ul(
        style = "margin: 0; padding-left: 20px; font-size: 12px;",
  ...
  output$gsea_per_gene_container <- renderUI({
    ...
    if (is.null(res_by_gene) || length(res_by_gene) == 0) {
      return(NULL)
    }
    ...
    for (i in seq_along(genes)) {
      local({
        ...
        output[[paste0("gsea_table_", ii)]] <- DT::renderDataTable({
          ...
          if (nrow(out) == 0) {
            return(DT::datatable(data.frame(Message = "No enriched pathways."), rownames = FALSE))
          }
        ...
        output[[paste0("download_gsea_plot_jpg_", ii)]] <- downloadHandler(
          ...
          content = function(file) {
            ...
            tryCatch(
              {
                p <- enrichplot::gseaplot2(result,
                  geneSetID = top_terms, title = paste("GSEA for", g),
