...
server_results <- function(input, output, session, rv) {
    ...
    tryCatch(
      {
        ...
        if (method == "deseq2") {
          if (!requireNamespace("DESeq2", quietly = TRUE)) {
            showNotification(
              tags$div(
                icon("exclamation-triangle"), tags$strong(" DESeq2 not installed."),
            ...
            showNotification(
              tags$div(
                icon("exclamation-triangle"), tags$strong(" Raw counts not available."),
        ...
        if (method == "deseq2") {
          withProgress(message = "DESeq2 analysis...", value = 0, {
            ...
            if (length(common_samples) < 3) {
              showNotification(
                tags$div(
                  icon("exclamation-triangle"), tags$strong(" Too few samples."),
            ...
            res <- DESeq2::results(dds,
              contrast = c("Condition", "Disease", "Normal"),
          ...
          withProgress(message = "limma DE analysis...", value = 0, {
            if (is.null(rv$batch_corrected) || is.null(rv$unified_metadata)) {
              showNotification(
                tags$div(
                  icon("exclamation-triangle"), tags$strong(" Missing data."),
      ...
      error = function(e) {
        showNotification(
          tags$div(
            icon("times-circle"), tags$strong(" DE analysis failed: "),
            conditionMessage(e)
          ),
  ...
  output$total_degs <- renderInfoBox({
    ...
    infoBox("Total DEGs", n,
      icon = icon("star", class = "fa-2x"),
  ...
  output$up_genes <- renderInfoBox({
    ...
    infoBox("Up-regulated", n,
      icon = icon("arrow-up", class = "fa-2x"),
      color = "red", fill = TRUE
    )
  })
  ...
  output$down_genes <- renderInfoBox({
    ...
    infoBox("Down-regulated", n,
      icon = icon("arrow-down", class = "fa-2x"),
  ...
  output$de_pipeline_verification <- renderUI({
    ...
    tags$div(
      ...
      tags$p(
        ...
        if (batch_done) {
          paste0(batch_label, " — applied to the expression matrix used for heatmaps, WGCNA, and downstream steps.")
        } else {
          "Not applied (single dataset or Step 5 skipped)."
        }
      ),
      tags$p(
    ...
    tryCatch(
      {
        ...
        if (length(valid_genes) < 2) {
          ...
          text(0.5, 0.5,
            paste0(
              "Cannot generate heatmap: only ", length(valid_genes),
  ...
  output$top_degs_table <- renderDT({
    ...
    top <- head(rv$sig_genes[
      order(rv$sig_genes$adj.P.Val),
