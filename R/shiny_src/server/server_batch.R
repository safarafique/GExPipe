...
server_batch <- function(input, output, session, rv) {
  ...
  output$batch_timer <- renderText({
    if (!isTRUE(rv$batch_running) || is.null(rv$batch_start)) {
      return("00:00")
    }
  ...
  output$genes_after_filter <- renderInfoBox({
    ...
    infoBox("Genes After Filter", n,
      icon = icon("filter", class = "fa-2x"),
  ...
  output$filter_info <- renderText({
    ...
    paste0(
      "Removing ", percent_remove, "% of genes (", format(n_remove, big.mark = ","),
  ...
  output$variance_cutoff <- renderInfoBox({
    ...
    infoBox("Variance Cutoff", val,
      icon = icon("chart-line", class = "fa-2x"),
  ...
  observeEvent(input$apply_batch, {
    ...
    if (!isTRUE(rv$groups_applied)) {
      showNotification(
        tags$div(
          icon("exclamation-triangle"), tags$strong(" Step 4 required:"),
  ...
  output$pca_before_dataset <- renderPlot({
    ...
    ggplot(df, aes(x = theta, y = r, color = Dataset)) +
      ...
      scale_x_continuous(
        limits = c(-pi, pi), breaks = c(-pi, -pi / 2, 0, pi / 2, pi),
        labels = c("-180°", "-90°", "0°", "90°", "180°")
      ) +
      scale_y_continuous(limits = c(0, NA)) +
      theme_bw(base_size = 14) +
      labs(
        title = "Before Batch Correction - By Dataset",
  ...
  output$pca_before_condition <- renderPlot({
    ...
    if (all(is.na(df$Condition)) || length(unique(df$Condition[!is.na(df$Condition)])) < 2) {
      ggplot(df, aes(x = theta, y = r)) +
        ...
        theme_bw(base_size = 14) +
        labs(
          title = "Before Batch Correction - By Condition",
      ...
      ggplot(df, aes(x = theta, y = r, color = Condition)) +
        geom_point(size = 3.5, alpha = 0.7) +
        scale_color_manual(
          values = c("Normal" = "#3498db", "Disease" = "#e74c3c", "None" = "#95a5a6"),
          na.value = "gray60"
        ) +
        coord_polar(theta = "x", start = -pi / 2, direction = 1) +
        scale_x_continuous(
  ...
  output$pca_after_dataset <- renderPlot({
    ...
    ggplot(df, aes(x = theta, y = r, color = Dataset)) +
      ...
      theme_bw(base_size = 14) +
      labs(
        title = "After Batch Correction - By Dataset",
  ...
  output$pca_after_condition <- renderPlot({
    ...
    if (all(is.na(df$Condition)) || length(unique(df$Condition[!is.na(df$Condition)])) < 2) {
      ggplot(df, aes(x = theta, y = r)) +
        ...
        theme_bw(base_size = 14) +
        labs(
          title = "After Batch Correction - By Condition",
          subtitle = "Conditions not yet assigned (circular)",
          x = "Angle (PC1–PC2)", y = "Radius"
        ) +
        theme(
      ...
      p <- tryCatch(
        {
          ...
          if (all(is.na(df$Condition)) || length(unique(df$Condition[!is.na(df$Condition)])) < 2) {
            ggplot2::ggplot(df, ggplot2::aes(x = theta, y = r)) +
              ggplot2::geom_point(size = 3.5, alpha = 0.7, color = "gray60") +
              ggplot2::coord_polar(theta = "x", start = -pi / 2, direction = 1) +
              ggplot2::scale_x_continuous(limits = c(-pi, pi), breaks = c(-pi, -pi / 2, 0, pi / 2, pi), labels = c("-180°", "-90°", "0°", "90°", "180°")) +
              ggplot2::scale_y_continuous(limits = c(0, NA)) +
              ggplot2::theme_bw(base_size = 14) +
            ...
            ggplot2::ggplot(df, ggplot2::aes(x = theta, y = r, color = Condition)) +
              ggplot2::geom_point(size = 3.5, alpha = 0.7) +
  ...
  batch_hclust_to_file <- function(file, dev_fun, before = TRUE) {
    ...
    if (ncol(expr_mat) > n_samples) {
      set.seed(123)
      sample_idx <- sample(seq_len(ncol(expr_mat)), n_samples)
      expr_subset <- expr_mat[, sample_idx]
      metadata_subset <- rv$unified_metadata[sample_idx, ]
    } else {
    ...
    tryCatch(
      {
        pheatmap::pheatmap(cor_matrix_ordered,
          cluster_rows = FALSE, cluster_cols = FALSE, annotation_col = annotation_col, annotation_row = annotation_col,
  ...
  output$download_pvca_before_png <- downloadHandler(
    ...
    content = function(file) {
      p <- batch_pvca_to_plot(before = TRUE)
      if (!is.null(p)) batch_save_ggplot(p, file, "png")
    }
  )
