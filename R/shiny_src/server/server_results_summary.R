...
server_results_summary <- function(input, output, session, rv) {
  ...
  output$results_summary_batch_before_after <- renderPlot(
    {
      ...
      if (is.null(before) || is.null(after) || is.null(meta)) {
        plot.new()
        text(0.5, 0.5, "Batch correction data needed (Step 5).", cex = 1, col = "gray40")
      ...
      if (length(common) < 3) {
        plot.new()
        text(0.5, 0.5, "Too few common samples.", cex = 1)
        return(invisible(NULL))
      }
  ...
  output$results_summary_roc_plot <- renderPlot(
    {
      if (is.null(rv$ml_common_genes) || length(rv$ml_common_genes) == 0 || is.null(rv$extracted_data_ml) || is.null(rv$wgcna_sample_info)) {
        plot.new()
        text(0.5, 0.5, "Run ML (Step 10) and ROC (Step 11) for AUC curves.", cex = 1, col = "gray40")
        return(invisible(NULL))
      }
      ...
      if (length(common_samples) < 3) {
        plot.new()
        text(0.5, 0.5, "Too few samples.", cex = 1)
        return(invisible(NULL))
      }
  ...
  output$results_summary_nomogram_ui <- renderUI({
    if (!isTRUE(rv$nomogram_complete)) {
      return(tags$p(style = "color: #7f8c8d;", "Run Diagnostic Nomogram (Step 12) to see summary here."))
    }
  ...
  output$results_summary_nomogram_plot <- renderPlot(
    {
      if (!isTRUE(rv$nomogram_complete) || is.null(rv$nomogram_model) || is.null(rv$nomogram_train_data) || is.null(rv$nomogram_available_genes) || length(rv$nomogram_available_genes) == 0) {
        plot.new()
        text(0.5, 0.5, "Run Nomogram (Step 12) to see plot.", cex = 1, col = "gray40")
        return(invisible(NULL))
      }
      tryCatch(
        {
        ...
        error = function(e) {
          plot.new()
          text(0.5, 0.5, paste("Plot error:", conditionMessage(e)), cex = 0.8, col = "red")
        }
      )
    },
    height = 260
  ...
  output$results_summary_volcano <- renderPlot(
    {
      if (is.null(rv$de_results)) {
        plot.new()
        text(0.5, 0.5, "Run DE analysis (Step 6) to see volcano plot.", cex = 1.1)
        return(invisible(NULL))
      }
      tryCatch(
        {
  ...
  output$results_summary_de_heatmap <- renderPlot(
    {
      if (is.null(rv$de_results) || is.null(rv$batch_corrected)) {
        plot.new()
        text(0.5, 0.5, "Run DE analysis to see heatmap.", cex = 1.1)
        return(invisible(NULL))
      }
      tryCatch(
        {
          ...
          pheatmap::pheatmap(expr_scaled,
            annotation_col = annot, color = colorRampPalette(c("#3498db", "white", "#e74c3c"))(100),
  ...
  output$results_summary_soft_threshold <- renderPlot(
    {
      if (is.null(rv$soft_threshold)) {
        plot.new()
        text(0.5, 0.5, "Run soft threshold (Step 7) to see plot.", cex = 1.1)
        return(invisible(NULL))
      }
  ...
  output$results_summary_sample_tree <- renderPlot(
    {
      if (is.null(rv$wgcna_sample_tree)) {
        plot.new()
        text(0.5, 0.5, "Prepare WGCNA data to see sample tree.", cex = 1.1)
        return(invisible(NULL))
      }
      ...
      op <- par(
        mar = c(2, 3.5, 3, 1), bg = "#f8fafb", fg = "#2c3e50", xpd = NA,
  ...
  output$results_summary_wgcna_dendro <- renderPlot(
    {
      if (is.null(rv$geneTree) || is.null(rv$moduleColors)) {
        plot.new()
        text(0.5, 0.5, "Build WGCNA network to see dendrogram.", cex = 1.1)
        return(invisible(NULL))
      }
      if (!requireNamespace("WGCNA", quietly = TRUE)) {
        par(bg = "white")
        plot.new()
        text(0.5, 0.5, "WGCNA required", cex = 1.2)
        return(invisible(NULL))
      }
  ...
  output$results_summary_module_trait <- renderPlot(
    {
      if (is.null(rv$moduleTraitCor) || is.null(rv$trait_data)) {
        par(bg = "white")
        plot.new()
        text(0.5, 0.5, "Calculate module-trait (Step 7) to see heatmap.", cex = 1)
        return(invisible(NULL))
      }
      if (!requireNamespace("WGCNA", quietly = TRUE)) {
        par(bg = "white")
  ...
  output$results_summary_go_plot <- renderPlot(
    {
      ...
      if (is.null(go_obj) || !inherits(go_obj, "enrichResult")) {
        plot.new()
        text(0.5, 0.5, "No GO enrichment results", cex = 1.2)
        return(invisible(NULL))
      }
      tryCatch(
        {
          ...
          if (nrow(df) == 0) {
            plot.new()
            text(0.5, 0.5, "No significant GO terms", cex = 1.2)
            return(invisible(NULL))
          }
  ...
  output$results_summary_kegg_plot <- renderPlot(
    {
      ...
      if (is.null(k)) {
        plot.new()
        text(0.5, 0.5, "No KEGG enrichment results", cex = 1.2)
        return(invisible(NULL))
      }
      tryCatch(
        {
          ...
          ggplot2::ggplot(df, ggplot2::aes(x = Description, y = Count, fill = p.adjust)) +
            ...
            ggplot2::labs(title = "KEGG Enrichment", x = NULL, y = "Count")
        },
        error = function(e) {
          plot.new()
          text(0.5, 0.5, paste("Error:", conditionMessage(e)), cex = 0.9, col = "red")
  ...
  output$results_summary_ppi_plot <- renderPlot(
    {
      if (is.null(rv$ppi_graph)) {
        plot.new()
        text(0.5, 0.5, "Run PPI (Step 9) to see network.", cex = 1.1)
        return(invisible(NULL))
      }
      tryCatch(
        {
          ...
          if (n_max < 2) {
            plot.new()
            text(0.5, 0.5, "Too few nodes for PPI plot.", cex = 1.1)
            return(invisible(NULL))
          }
          ...
          on.exit(par(op), add = TRUE)
          vlab <- igraph::V(g_sub)$SYMBOL
          if (is.null(vlab)) vlab <- igraph::V(g_sub)$name
          igraph::plot.igraph(g_sub,
            layout = layout_fr, vertex.label = vlab,
  ...
  output$results_summary_ml_venn <- renderPlot(
    {
      ...
      if (n_sets > 5) {
        ...
        if (length(all_genes) == 0) {
          plot.new()
          text(0.5, 0.5, "No genes", cex = 1, col = "gray40")
          return(invisible())
        }
        ...
        tryCatch(
          {
            UpSetR::upset(upset_df,
              sets = colnames(upset_df), keep.order = TRUE, order.by = "freq",
          ...
          error = function(e) {
            plot.new()
            text(0.5, 0.5, "UpSet error", cex = 1, col = "red")
          }
        )
      } else {
        ...
        vp <- VennDiagram::venn.diagram(
          x = sets, category.names = cat_names, filename = NULL, output = TRUE,
  ...
  output$results_summary_gsea_plot <- renderPlot(
    {
      by_gene <- rv$gsea_results_by_gene
      if (is.null(by_gene) || length(by_gene) == 0) {
        plot.new()
        text(0.5, 0.5, "Run GSEA (Step 12) to see enrichment plot.", cex = 1)
        return(invisible(NULL))
      }
      result <- by_gene[[1]]
      if (!inherits(result, "enrichResult")) {
        plot.new()
        text(0.5, 0.5, "No GSEA result for first gene.", cex = 1)
        return(invisible(NULL))
      }
      tryCatch(
        {
          ...
          if (is.null(df) || nrow(df) == 0) {
            plot.new()
            text(0.5, 0.5, "No pathways for first gene.", cex = 1)
            return(invisible(NULL))
          }
  ...
  output$results_summary_immune <- renderUI({
    ...
    if (is.null(M) || nrow(M) == 0) {
      return(tags$p(style = "color: #7f8c8d;", "Run Immune deconvolution (Step 13) to see results here."))
    }
    n_samp <- nrow(M)
