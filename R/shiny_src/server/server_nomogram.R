...
server_nomogram <- function(input, output, session, rv) {
  ...
  output$nomogram_validation_mode_ui <- renderUI({
    ...
    if (mode == "external") {
      ...
      tags$div(
        ...
        if (has_ext) {
          paste0(
            "Model trained on ALL training data. External dataset (",
  ...
  observeEvent(input$run_nomogram, {
    ...
    if ((is.null(rv$roc_selected_genes) || length(rv$roc_selected_genes) == 0) &&
      ...
      showNotification(
        tags$div(
          icon("exclamation-triangle"), tags$strong(" Gene list required:"),
    ...
    if (mode == "external" && is.null(rv$external_validation_expr)) {
      showNotification(
        tags$div(
          icon("exclamation-triangle"), tags$strong(" External validation data required:"),
    ...
    if (!is.null(sample_info$SampleID)) {
    ...
    if (length(u) < 2) {
      msg <- if (length(u) == 0) {
        paste0("No group values in column '", group_col, "'.")
      } else {
        paste0("Only one group found: '", u[1], "'. Need exactly two groups.")
      }
      ...
      if (length(ext_genes_available) < 1) {
        showNotification(
          tags$div(
            ...
            tags$br(),
            tags$small(paste0(
              "Model genes: ", paste(head(available_genes, 5), collapse = ", "),
    ...
    nomogram_model <- tryCatch(
      ...
      error = function(e) {
        showNotification(paste("Model failed:", e$message), type = "error", duration = 8)
        NULL
      }
    )
    ...
    cal_validation <- validation_data %>%
      ...
      dplyr::summarise(
        Predicted = mean(.data$Predicted_Prob), Observed = mean(.data$Outcome), N = dplyr::n(),
